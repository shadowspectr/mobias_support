...
# support_router должен быть первым, чтобы обработать кнопку поддержки
dp.include_router(support_router)  # Роутер поддержки ПЕРВЫЙ
...
KNOWN_BUTTON_TEXTS = {
    ...
    "❓ Обратиться в поддержку",  # Эта кнопка будет обработана в support_router
    "🏬 Адреса магазинов",
    "🎁 Актуальные акции"
}
...
# ИСПРАВЛЕННЫЙ Fallback обработчик - убираем проверку на известные кнопки
@dp.message(lambda message: (
    message.chat.id != SUPPORT_TICKETS_CHAT_ID and 
    message.chat.type == "private"
    # Убираем проверку на support_user_id, так как она неправильная
))
async def fallback_handler(message: types.Message, bot: Bot):
    # УБИРАЕМ эту проблемную проверку, которая блокировала обработку
    # if message.text and message.text.strip() in KNOWN_BUTTON_TEXTS:
    #     return
    
    # Проверяем, не обрабатывается ли уже это сообщение другими обработчиками
    # Это более безопасный способ
    if message.text and message.text.strip() in KNOWN_BUTTON_TEXTS:
        # Логируем, что кнопка должна обрабатываться другим обработчиком
        logging.info(f"Кнопка '{message.text.strip()}' должна обрабатываться специальным обработчиком")
        return
