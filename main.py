...
from src.broadcast import send_random_ad, send_ad_to_user

# Removed: from aiogram.filters import Text
...
# FIXED: Replaced Text(...) with F.text == "..."
...
async def forward_answer_from_support(message: types.Message, bot: Bot):
    if message.reply_to_message:
        try:
            ...
            logging.error(f"Could not parse user_id from reply: {e}")
            await message.reply("Не удалось извлечь ID пользователя из сообщения, на которое вы отвечаете. Пожалуйста, убедитесь, что вы отвечаете на правильное сообщение.")
...
# FIXED: Updated to modern F.location filter
...
# Note: This router is defined but has no handlers attached to it.
# The include_router call is currently not doing anything.
